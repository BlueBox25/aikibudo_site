...
/**
 * Links through wa.me so the destination follows the device: WhatsApp Web on a
 * computer, the app on a phone, the store when it is not installed yet.
 */
export default function WhatsAppButton({ phone, label = 'WhatsApp', size = 'md', className }) {
  const href = waLink(phone)
  if (!href) return null

  return (
    <a
      className={cx(styles.button, size === 'sm' && styles.small, className)}
      href={href}
      target="_blank"
      rel="noreferrer noopener"
      aria-label={`Scrie pe WhatsApp la ${phone}`}
    >
      <Glyph />
      {label}
    </a>
